...
- **房間管理者與角色權限**：房主在建立房間後會立即收到 `Baha-Admin-XXXX-XXXX` 的管理金鑰，畫面會在右上方顯示一個「管理金鑰橫幅」，每次金鑰產生時（建立或使用 `/adminkey`）都能立即看見並複製；這組金鑰連同儲存於瀏覽器 `localStorage` 的匿名 ID 會綁定在同一台裝置；即使重新整理、重新連線甚至網頁重啟，房主仍能透過 `/auth <金鑰>` 恢復房間管理權限（指令可在該房間內使用 `/rename`, `/public`, `/private`, `/clear`, `/delete`, `/ban`, `/kick`, `/mute`, `/announce`, `/adminkey` 等）。這種角色與權限模型讓匿名使用者也能合理地維護秩序，不需要公開真實身份。
...
1. **前端介面層**：負責畫面、互動、白板與聊天室顯示。
2. **Socket 事件層**：負責處理建立房間、聊天、討論串、指令與狀態同步。
3. **資料與工具層**：負責資料模型、房間列表排序、投票與共用設定。

```mermaid
flowchart TB
  U[使用者瀏覽器] --> P[public/index.html]
  P --> S[public/script.js]
  P --> C[public/style.css]
  P --> I[public/i18n.js]
  P --> SW[public/sw.js]

  S -->|Socket.io| SV[server.js]
...
| [public/index.html](public/index.html) | 前端骨架 | 定義大廳、聊天室、桌機白板、公告、贊助與更新提示的 DOM 結構 |
| [public/script.js](public/script.js) | 前端互動邏輯 | 處理 Socket 事件、Markdown、討論串、Typing Indicator、白板組件與通知 |
| [public/style.css](public/style.css) | 視覺與版面 | 控制聊天室泡泡、白板卡片、RWD、桌機版三欄佈局與整體風格 |
| [public/i18n.js](public/i18n.js) | 多國語系 | 提供介面文案翻譯 |
| [public/sw.js](public/sw.js) | PWA 快取 | 負責離線快取與版本更新提醒 |
...
| [create-hash.js](create-hash.js) | 雜湊工具 | 提供識別碼或其他輔助雜湊功能 |

### 架構理解

- 這個專案已經不是單純聊天室，而是「匿名聊天室 + 房間管理 + 子討論串 + 桌機白板」的混合應用。
- 前端負責畫面與互動，後端負責狀態與資料，兩者透過 Socket.io 即時同步。
- 目前的資料拆分方式已經很適合後續維護，因為聊天室、房間、討論串、指令與投票都各自有獨立模組。
...
- `/adminkey`：只有房主可用，可立即在畫面上重新產生/查詢目前的 `Baha-Admin-XXXX-XXXX`，並提供一鍵複製功能，避免錯過系統訊息中的代碼。
